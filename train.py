...
from dataset_xe import CustomDataset, cartesian_mask
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_data in enumerate(pbar):
            # Adjust learning rate per iteration (or per epoch)
            current_iter = epoch + batch_idx / num_batches
            lr = adjust_learning_rate(optimizer, current_iter, args)
        ...
        epoch_train_loss = train_loss_epoch / num_batches if num_batches > 0 else 0.0
        writer.add_scalar("Loss/train_epoch", epoch_train_loss, epoch)
        print(f"Epoch {epoch+1} training loss: {epoch_train_loss:.6f}")
