...
def scaled_dot_product_attention(query, key, value, dropout_p=0.0) -> torch.Tensor:
    ...
    # 使用 query 的设备，而不是硬编码 .cuda()
    device = query.device
    attn_bias = torch.zeros(query.size(0), 1, L, S, dtype=query.dtype, device=device)

    # 使用 torch.amp.autocast 代替 torch.cuda.amp.autocast，以支持不同设备
    with torch.amp.autocast(device_type=str(device.type), enabled=False):
...
class JiT(nn.Module):
    ...
    def forward(self, x, t, cond):
        ...
        # forward JiT
        x = self.x_embedder(torch.cat((x, cond), dim=1))
        x += self.pos_embed
